...
from main import *
from software.util_functions import *
import random
import numpy as np
import math
...
if args.data_name is not None:
    # Load dataset
    dataNet_ori = np.load(os.path.join(args.file_dir, 'software/data/dream/ind.{}.csc'.format(args.data_name)), allow_pickle=True)
    dataGroup = np.load(os.path.join(args.file_dir, 'software/data/dream/ind.{}.allx'.format(args.data_name)), allow_pickle=True)
    dataNet_agent0 = np.load(args.file_dir+'/software/data/dream/'+args.data_name+'_pmatrix_'+str(args.pearson_net)+'.npy', allow_pickle=True).tolist()
    dataNet_agent1 = np.load(args.file_dir+'/software/data/dream/'+args.data_name+'_mmatrix_'+str(args.mutual_net)+'.npy', allow_pickle=True).tolist()
